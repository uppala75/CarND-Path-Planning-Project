          ...
          // j[1] is the data JSON object
            ...
            for (int i=0; i < sensor_fusion.size(); i++){
              ...
              if(d < (2+4*lane+2) && d > (2+4*lane-2)){
                ...
                //reduce ref velocity so we dont crash into the car on the front and flag to change lanes if possible
                //ref_vel = 29.5; //mph
                  too_close = true;
                } 
              } else if (lane != 2 && d > (4*(lane+1)) && d < (4*(lane+2))){
                  // check if car is in my right lane
                  double vx = sensor_fusion[i][3];
                  double vy = sensor_fusion[i][4];
                  double check_speed = sqrt(vx*vx+vy*vy);
                  double check_car_s = sensor_fusion[i][5];
                  ...
                  // check if car is in my left lane
                  double vx = sensor_fusion[i][3];
                  double vy = sensor_fusion[i][4];
                  double check_speed = sqrt(vx*vx+vy*vy);
                  double check_car_s = sensor_fusion[i][5];
            ...
            if (too_close){
              //reduce speed when car ahead is too close
              ref_vel -= 0.224;

              //switch into left or right lane accordingly when there are no closeby cars in those lanes
          	...
          	//reference x, y, yaw states either we will reference the starting point as where the car is or at
            ...
            // TODO: define a path made up of (x,y) points that the car will visit sequentially every .02 seconds
          	...
          	//In frenet, add 30m evenly spaced points ahead of the starting reference
            ...
            for (int i = 1; i <= 50-previous_path_x.size(); i++){
              double N = (target_dist/(0.02*ref_vel/2.24)); //converting ref_vel to meters/sec
